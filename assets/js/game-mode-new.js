...
class PortfolioGame {
  ...
  generateInitialPlatforms() {
    ...
    // Info platforms at the start
    this.platforms.push({ 
      x: 250, y: 400, width: 100, height: 150, 
  ...
  generateNewPlatform() {
    ...
    // Add coin above platform sometimes
    if (Math.random() < 0.4) {
      this.floatingCoins.push({
        x: x + width/2 - 10,
        y: y - 70,
  ...
  respawn() {
    ...
    // Keep distance and coins for continuous play
  }
  ...
  update() {
    ...
    // Auto-scroll camera to follow player
    ...
    this.platforms.forEach(platform => {
      ...
      if (this.player.x + this.player.width > platformScreenX &&
          this.player.x < platformScreenX + platform.width &&
    ...
    this.floatingCoins.forEach(coin => {
      if (!coin.collected) {
        ...
        if (this.player.x + this.player.width > coinScreenX &&
            this.player.x < coinScreenX + coin.width &&
  ...
  drawUI() {
    ...
    // Distance counter
    ctx.fillStyle = '#000';
    ctx.fillRect(20, 15, 200, 45);
    ctx.strokeStyle = '#FFD700';
    ctx.lineWidth = 3;
    ctx.strokeRect(20, 15, 200, 45);
    
    ctx.fillStyle = '#FFD700';
    ctx.font = '12px "Press Start 2P"';
    ctx.textAlign = 'left';
    ctx.fillText(`DISTANCE`, 30, 35);
    ctx.font = '16px "Press Start 2P"';
    ctx.fillText(`${Math.floor(this.distance)}m`, 30, 52);
